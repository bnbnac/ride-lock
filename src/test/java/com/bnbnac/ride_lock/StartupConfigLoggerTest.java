...
class StartupConfigLoggerTest {
	...
	void logsEffectiveVirtualThreadsSetting() {
		...
		StartupConfigLogger logger = new StartupConfigLogger(dataSource, "redis", 5000, 20, true);
	...
	void logsFalseWhenVirtualThreadsDisabled() {
		HikariDataSource dataSource = mock(HikariDataSource.class);
		when(dataSource.getMaximumPoolSize()).thenReturn(10);
		StartupConfigLogger logger = new StartupConfigLogger(dataSource, "pessimistic", 5000, 20, false);
