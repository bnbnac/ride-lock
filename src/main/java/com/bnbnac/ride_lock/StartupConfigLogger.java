...
// 3주차 부하테스트에서 실행마다 바꿔가며 비교할 값(락 전략, 커넥션 풀 크기, 핫존 후보 반경/인원수)을
// 기동 완료 시점에 한 줄로 찍는다. @Value("${...:기본값}") 플레이스홀더는 프로퍼티 키를 오타내도
// 에러 없이 조용히 기본값으로 떨어지므로, 로그로 실제 적용된 값을 눈으로 확인하지 않으면 오타
// 한 번으로 그 실행의 측정 전체가 의도한 조건과 다른 채 조용히 무효가 될 수 있다.
@Component
public class StartupConfigLogger {
	...
	public void logEffectiveConfig() {
		...
		log.info("실행 설정 확인 - matching.lock-strategy={}, hikari.maximum-pool-size={}, "
						+ "matching.candidate-radius-meters={}, matching.candidate-limit={}",
				lockStrategy, maximumPoolSize, candidateRadiusMeters, candidateLimit);
	}
